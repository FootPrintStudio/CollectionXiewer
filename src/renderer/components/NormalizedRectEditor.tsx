...
export function NormalizedRectEditor({
  ...
  initialRect = null,
  onRectChange,
  hint = 'Drag on the image to draw a selection',
  className = 'marquee-crop'
}: Props) {
  const containerRef = useRef<HTMLDivElement>(null)
  const [containerSize, setContainerSize] = useState({ w: 0, h: 0 })
  const [naturalSize, setNaturalSize] = useState({ w: 0, h: 0 })
  const [selection, setSelection] = useState<CropRect | null>(initialRect)
  ...
  return (
    ...
    >
      ...
      {display ? (
        ...
        >
          {!selection ? <p className="marquee-crop__hint">{hint}</p> : null}
          {selectionPx ? (
            <div
              className="marquee-crop__selection subject-region-editor__selection"
              style={{
